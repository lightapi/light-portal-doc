...
Tutorials for setting up and operating a multi-repo workspace for coding
agents, rather than a single service.

- [Setup a local workspace user for coding agents on Ubuntu](./workspace/local-agent-setup.md):
  run terminal agents like OpenAI Codex CLI and Claude Code from an isolated
  Linux account so they cannot directly read your personal data in the
  normal desktop profile.
- [codebase-memory-mcp](./workspace/codebase-memory-mcp.md): install the
