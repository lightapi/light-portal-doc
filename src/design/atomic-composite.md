...
If you want to replace the existing connector, you can use the put request. 

```
curl --location --request PUT 'http://localhost:8083/connectors/outbox-connector/config' \
--header 'Content-Type: application/json' \
--data-raw '{
    "connector.class": "io.debezium.connector.postgresql.PostgresConnector",
    "tasks.max": "1",
